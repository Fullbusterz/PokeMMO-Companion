...
- **`abilities.json` hecho para las 5 regiones (649 Pokémon)**, scrapeado directamente de la PokeMMO Wiki (pokemmo.shoutwiki.com) — habilidad(es) normal(es) + habilidad oculta por Pokémon, con sección "Habilidades" visible en la pantalla de detalle. Solo Farfetch'd sin verificar (la propia wiki marca su ficha como rota). `movesets.json`/`locations.json`/`pokemmo_tiers.json` siguen siendo stubs vacíos — la regla de oro de abajo aplica en cuanto se rellenen. La wiki también tiene tablas limpias de movimientos y ubicaciones salvajes, listas para el mismo tipo de scraping. **Ver "Ideas pendientes" abajo — hay una fuente candidata muy buena para `locations.json` (guías de farmeo de seviichamp.blogspot.com).** Este es el siguiente bloque de trabajo de la Fase 2.
