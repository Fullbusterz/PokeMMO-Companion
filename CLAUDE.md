...
- **`abilities.json` de Kanto hecho (151 entradas)**, scrapeado directamente de la PokeMMO Wiki (pokemmo.shoutwiki.com) — habilidad(es) normal(es) + habilidad oculta por Pokémon, con sección "Habilidades" ya visible en la pantalla de detalle. Johto/Hoenn/Sinnoh/Teselia y `movesets.json`/`locations.json`/`pokemmo_tiers.json` siguen siendo stubs vacíos — la regla de oro de abajo aplica en cuanto se rellenen. **Ver "Ideas pendientes" abajo — hay una fuente candidata muy buena para `locations.json` (guías de farmeo de seviichamp.blogspot.com).** Este es el siguiente bloque de trabajo de la Fase 2.
